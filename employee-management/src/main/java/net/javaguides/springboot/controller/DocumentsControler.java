...
if(file.getSize()!=0) {
    ...
    public void downloadFile2(@Param("id") Long id, Model model , HttpServletResponse response) throws IOException {
        Optional<PlotRegistration> plot=plotResidenceService.findById(id);
        if(!plot.isEmpty()){
            PlotRegistration p=plot.get();
        Documents temp = documentsService.findByPlotNum(p.getPlotNumber());
        if (temp != null) {
            Documents docu = temp;
            response.setContentType("application/octet-stream");
            String headerKey = "Content-Disposition";
