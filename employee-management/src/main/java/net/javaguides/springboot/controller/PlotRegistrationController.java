...
//        return "index" ;
    ...
    public String deletePlotandDocuments(@PathVariable (value = "id")  Long id,Model model){
       Optional<PlotRegistration> plot=plotResidenceService.findById(id);
       if(!plot.isEmpty()){
           PlotRegistration p=plot.get();
           Documents docu=documentsService.findByPlotNum(p.getPlotNumber());
           plotResidenceService.delete(p);
           documentsService.deleteDocu(docu);
       }
        return "redirect:/";
    }
}
